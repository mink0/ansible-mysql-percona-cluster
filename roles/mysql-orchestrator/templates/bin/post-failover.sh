...
mysql ${proxysql_login} -v -e "DELETE FROM mysql_servers WHERE hostgroup_id=0 \
...
mysql ${proxysql_login} -v -e "INSERT INTO mysql_servers(hostgroup_id,hostname,port,status) \
